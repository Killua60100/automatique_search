...
def main():
    ...
    if not files:
        print("Aucune archive trouvée dans legi_download.")
        return
    files.sort(reverse=True)
    filepath = os.path.join(download_dir, files[0])
    extract_path = extract_tar_file(filepath)  # extraction ira dans legi_extract
    xml_files = find_xml_files(extract_path)
    print(f" {len(xml_files)} fichiers XML trouvés")
    parse_articles(xml_files, max_articles=None, output_path="data/articles_legi.json")
